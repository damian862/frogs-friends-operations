(function(){
  ...
  function forceCalendarVisible(){
    const panel=document.getElementById('bookingTabCalendar');
    const tab=document.querySelector('.booking-tab[data-btab="calendar"]');
    if(!panel||!tab)return;
  ...
  window.setBookingTab=function(tab){
    if(tab==='calendar'){
      if(prevTab)prevTab('calendar');
      setTimeout(forceCalendarVisible,0);
      setTimeout(forceCalendarVisible,80);
      return;
    }
  ...
  document.addEventListener('click',ev=>{
    ...
    if(income){staffingCustomerTouched=false;setTimeout(()=>{fixStaffingFilter();if(window.renderLifeguardServices)window.renderLifeguardServices()},80)}
  });
