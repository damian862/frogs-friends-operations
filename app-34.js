(function(){
  ...
  function ensureQueue(){const panel=document.getElementById('bookingTabIncome');if(!panel||!canView())return null;let wrap=document.getElementById('financeQueue');if(!wrap){wrap=document.createElement('div');wrap.id='financeQueue';wrap.className='finance-queue';wrap.innerHTML=`<div class="finance-queue-head"><div><h2>Finance Queue</h2><p>Approved monthly pool-hire statements waiting to be invoiced in the school accounts system.</p></div><div class="finance-queue-summary"><span>Waiting</span><b id="financeQueueCount">0</b><span id="financeQueueTotal">£0.00</span></div></div><div id="financeQueueBody" class="card"><div class="muted">Loading approved statements…</div></div>`;const monthly=document.getElementById('monthlyBilling');if(monthly)monthly.insertAdjacentElement('afterend',wrap);else panel.appendChild(wrap)}return wrap}
  async function loadQueue(){const wrap=ensureQueue();if(!wrap)return;const body=document.getElementById('financeQueueBody');const {data,error}=await sb.from('school_invoice_batches').select('*').eq('status','ready').order('usage_month',{ascending:true});if(error){body.innerHTML=`<div class="err">${e(error.message)}</div>`;return}const rows=data||[];document.getElementById('financeQueueCount').textContent=rows.length;document.getElementById('financeQueueTotal').textContent=money(rows.reduce((a,r)=>a+Number(r.total_amount||0),0));body.innerHTML=rows.length?`<div class="finance-table-wrap"><table class="finance-table"><thead><tr><th>Organisation</th><th>Site</th><th>Month</th><th>Net</th><th>VAT</th><th>Total</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${e(hn(r.hirer_id)||'Unknown organisation')}</b></td><td>${e(sn(r.site_id)||'')}</td><td>${e(monthLabel(r.usage_month))}</td><td>${money(r.net_amount)}</td><td>${money(r.vat_amount)}</td><td><b>${money(r.total_amount)}</b></td><td><div class="finance-actions"><button class="s" onclick="financeViewStatement('${r.id}')">View statement</button>${canAction()?`<button class="p" onclick="financeMarkInvoiced('${r.id}')">Mark invoiced</button>`:''}</div></td></tr>`).join('')}</tbody></table></div>`:'<div class="finance-empty"><b>No statements waiting for Finance.</b><span>Statements will appear here after the Lettings Manager has approved them and they have been marked Ready for Finance.</span></div>'}
  let queueLoadTimer=null;
  function scheduleQueueLoad(delay=120){if(queueLoadTimer!==null)clearTimeout(queueLoadTimer);queueLoadTimer=setTimeout(()=>{queueLoadTimer=null;loadQueue()},delay)}
  window.financeViewStatement=id=>{if(typeof window.viewBillingStatement==='function')window.viewBillingStatement(id)};
    ...
    if(typeof window.renderIncomeSummary==='function'){
      setTimeout(()=>{
        ...
        setTimeout(()=>{
          ...
          const scope=document.getElementById('siteScope')||document.getElementById('siteSelect');if(scope&&selectedViewingSite&&[...scope.options].some(o=>o.value===selectedViewingSite))scope.value=selectedViewingSite;
          if(typeof window.renderMonthlyBilling==='function')window.renderMonthlyBilling();
        },80);
      },0);
    }
  };
  const priorIncome=window.renderIncomeSummary;window.renderIncomeSummary=function(){const out=priorIncome();scheduleQueueLoad();return out};const priorBilling=window.renderMonthlyBilling;window.renderMonthlyBilling=function(){const out=priorBilling();scheduleQueueLoad();return out};
