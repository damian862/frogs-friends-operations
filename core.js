...
function originatingEnquiryLink(kind,id) {
  return typeof window.originatingCommercialEnquiryLink === 'function' ? window.originatingCommercialEnquiryLink(kind,id) : '';
}
function renderBookingTables() {
  let row = x => `<tr data-booking-id="${x.id}" class="${x.status === 'cancelled' ? 'booking-row-cancelled' : ''}"><td>${e(ukDate(x.booking_date))}</td><td><b>${e(bookingOrg(x))}</b></td><td>${e(x.title)}${bookingStatusHtml(x)}</td><td>${e(sn(x.site_id))}</td><td>${String(x.start_time || '').slice(0, 5)}–${String(x.end_time || '').slice(0, 5)}</td><td>${bookingChargeHtml(x)}</td><td><button class=link onclick="editBooking('${x.id}')">Edit</button>${originatingEnquiryLink('booking',x.id)}</td></tr>`;
